    ...
    <div className="flex min-h-screen w-full relative">
      ...
      {viewMode === 'line' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.sections.map(section => {
            // Compter tous les membres (y compris ceux des sous-sections)
            const countAllMembers = (s: Section): number => {
              let count = s.members.length;
              if (s.subsections) {
            ...
            // Collecter les premiers membres (de la section et sous-sections)
            const collectFirstMembers = (s: Section, max: number): Person[] => {
              const members: Person[] = [];
              members.push(...s.members);
              if (members.length < max && s.subsections) {
                for (const sub of s.subsections) {
                  members.push(...collectFirstMembers(sub, max - members.length));
            ...
            const displayMembers = collectFirstMembers(section, 3);
              ...
              <div
                ...
                onClick={() => {
                  if (displayMembers.length > 0) {
                    handlePersonClick(displayMembers[0]);
                  }
                }}
              >
                <h3 className="font-semibold text-lg mb-2">{section.title}</h3>
                
                {/* Informations sur les membres et sous-sections */}
                <div className="text-sm text-muted-foreground mb-3 space-y-1">
                  <div>{totalMembers} membre{totalMembers > 1 ? 's' : ''}</div>
                  {subsectionCount > 0 && (
                    <div className="text-xs">
                      {subsectionCount} {subsectionCount > 1 ? 'commissions' : 'commission'}
                    </div>
                  )}
                </div>
                
                {/* Affichage des premiers membres */}
